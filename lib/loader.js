...
module.exports = app => {
  const defaultConfig = {
    // model 的路径, 参数为 app
    swaggerMountPath: '/swagger.json',
    swaggerDoc: {},
    wrapScript: modelRestJson => modelRestJson,
  };
  ...
  const loaderModels = modelConfigFiles.map(filePath => {
    ...
    const warpModelConfig = config.wrapScript(modelConfig);
  ...
  app.router.get(config.swaggerMountPath, function(ctx) {
    ctx.body = swaggerObj.swaggerRoot;
  });
