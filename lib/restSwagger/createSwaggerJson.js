...
class RestSwagger {
  ...
  createDefinitions(loaderModels = []) {
    ...
    loaderModels.forEach(loadModel => {
      ...
      // TODO: settings.definition => definition
      const defInfo = isFunction
        ? model.createDefinition()
        : _.result(loadModel, 'settings.definition') || {};

      const validateDef = _.pick(
        _.assign(_.cloneDeep(swaggerTemplate.defaultDefinitionItem), defInfo),
        pickKeys
      );
